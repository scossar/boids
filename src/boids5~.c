...
static t_class *boids5_class = NULL;
...
static void wavetable_init(void)
{
  if (cos_table == NULL) {
    ...
    if (cos_table) {
      ...
      post("boids5~: initialized cosine table of size %d (plus 1 sample)", WAVETABLE_SIZE);
    } else {
      post("boids5~ error: failed to allocate memory for cosine table");
      return;
    }
  }
...
static void windowtable_init(void)
{
  if (window_table == NULL) {
    ...
    if (window_table) {
      ...
      post("boids5~: initialized window table of size %d (plus 1 sample)", WINDOWTABLE_SIZE);
    } else {
      post("boids5~: error: failed to allocate memory for window table");
      return;
    }
  }
...
static void wavetable_free(void)
{
  ...
  if (wavetable_reference_count <= 0 && cos_table != NULL) {
    ...
    cos_table = NULL;
    post("boids5~: freed cosine table");
...
static void cycle_divisions_init(t_boids5 *x)
{
  ...
  for (int i = 0; i < x->x_num_cycle_divisions; i++) {
    ...
    if (x->cycle_divisions[i].boid_indices == NULL) {
      pd_error(x, "boids5~: failed to allocate memory for cycle_division[%d].boid_indeces", i);
      return;
    }
...
static void cycle_division_update(t_boids5 *x) {
...
static void boids_init(t_boids5 *x)
{
  x->boids = (t_boid *)getbytes(sizeof(t_boid) * x->x_num_boids);
  if (x->boids == NULL) {
    pd_error(x, "boids5~: failed to allocate memory for boids");
    return;
  }
...
static void boids_update(t_boids5 *x)
{
  for (int i = 0; i < x->x_num_boids; i++) {
    t_boid *boid = &x->boids[i];
...
static void activate_division_boids(t_boids5 *x, t_cycle_division *div)
{
...
static void move_boid_to_division(t_boids5 *x, int boid_idx, t_cycle_division *new_div)
{
...
// and new)
static void apply_boid_threshold_rule(t_boids5 *x, t_cycle_division *div, int threshold)
{
...
static void apply_div_harmonizing_rule(t_boids5 *x, t_cycle_division *div)
{
...
static void apply_div_odd_even_rule(t_boids5 *x, t_cycle_division *div)
{
  int sum = 0;
  for (int i = 0; i < div->num_boids; i++) {
    t_boid *boid = &x->boids[div->boid_indices[i]]; 
...
static void boids5_dsp(t_boids5 *x, t_signal **sp)
{
  ...
  boids_update(x);
  dsp_add(boids5_perform, 4, x, sp[0]->s_vec, sp[1]->s_vec, sp[0]->s_length);
}

static void boids_free(t_boids5 *x)
{
...
static void *boids5_new(t_floatarg root_freq, t_floatarg num_boids)
{
  t_boids5 *x = (t_boids5 *)pd_new(boids5_class);
...
void boids5_tilde_setup(void)
{
  boids5_class = class_new(gensym("boids5~"),
                           (t_newmethod)boids5_new,
                           (t_method)boids_free,
                           sizeof(t_boids5),
                           CLASS_DEFAULT,
